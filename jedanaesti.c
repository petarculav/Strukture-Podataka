...
int ReadFromFile(countryPos hash[HASH_SIZE], char* filename)
{
    FILE* countries=NULL;
    ...
    while(strlen(temp)>0)
    {
        ...
        if(state==2)
        {
            ...
            if(tempCountry)
            {
                ...
                if(strlen(tempCountry -> name) < limit)
                    limit= strlen(tempCountry->name);

                for(int i=0;i<limit;i++)
                {
                    key += (tempCountry->name)[i];
                }

                key = key % HASH_SIZE;
                InsertCountry(hash[key], tempCountry);
...
int SearchCities(countryPos hash[HASH_SIZE])
{
    char* countryName=NULL;
    ...
    if (strlen(countryName) < limit)
        limit=strlen(countryName);
    ...
    toFind=FindCountry(hash[key],countryName);
    ...
    PrintLine();
    PrintLarger(toFind->root,popLimit);
